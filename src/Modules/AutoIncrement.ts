 ...
 * @LastEditTime: 2025-06-23 05:16:21
...
CounterSchema.statics.getNextSequence = async function (modelName: string, autoIncrement: string): Promise<unknown> {
    const isModel = await Counters.findOne({ modelName: modelName }).catch((err) => {
...
const Counters = model<ICounter, ICounterModel>('Counter', CounterSchema);
// src/utils/sequence.util.ts
